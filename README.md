...
- Изображение товара: фото с сайта, `srcset` 800/970 px, явные `width`/`height`, осмысленный `alt`.
- Название (`h1`), категория «Ароматизированный улун» бейджем над названием и блок с коротким описанием товара.
...
- Кнопка «В корзину»: обычное состояние (чёрная), при наведении (жёлтая), после клика на секунду «Добавлено».
- При переключении фасовки обновляются цена, старая цена, процент скидки, артикул и выбранное состояние.
- Адаптив под 1440, 768 и 375 px: на планшете две колонки сохраняются, на мобильном блок складывается в одну колонку, описание уходит под кнопку.
...
- [x] 375 px: одна колонка; порядок: фото, категория, название, фасовка, цена, кнопка, артикул, описание; горизонтального скролла нет.
